...
impl CompactSize {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        // match the value aggainst diff ranges
        match self.value {
            ...
            // catch everything larger than 4byts
            _ => {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        match bytes[0] {
            0xFD => {
                ...
                let decoded_value = u16::from_le_bytes([bytes[1], bytes[2]]) as u64;
                Ok((CompactSize { value: decoded_value }, 3))
            }
            0xFE => {
                if bytes.len() < 5 {
                    return Err(BitcoinError::InsufficientBytes);
                }
                let decoded_value = u32::from_le_bytes([bytes[1], bytes[2], bytes[3], bytes[4]]) as u64;
                Ok((CompactSize { value: decoded_value }, 5))
            }
            0xFF => {
                if bytes.len() < 9 {
                    return Err(BitcoinError::InsufficientBytes);
                ...
                Ok((CompactSize { value: decoded_value }, 9))
            }
            byte_prefix => Ok((CompactSize { value: byte_prefix as u64 }, 1)),
        }
    }
}
...
impl Script {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let script_data_len = compact_size.value as usize;
        let total = prefix_len + script_data_len;
        if bytes.len() < total {
            return Err(BitcoinError::InsufficientBytes);
        }
...
impl BitcoinTransaction {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (num_inputs, inputs_prefix_len) = CompactSize::from_bytes(&bytes[4..])?;
        let mut offset = 4 + inputs_prefix_len;
...
impl fmt::Display for BitcoinTransaction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "tx_version: {}", self.version)?;
        writeln!(f, "num inputs: {}", self.inputs.len())?;
        for (i, input) in self.inputs.iter().enumerate() {
            writeln!(f, "input[{}]:", i + 1)?;
            writeln!(f, "txid: {}", hex::encode(input.previous_output.txid.0))?;
            writeln!(f, "vout: {}", input.previous_output.vout)?;
            writeln!(f, "script len: {}", input.script_sig.bytes.len())?;
            writeln!(f, "script: {}", hex::encode(&input.script_sig.bytes))?;
            writeln!(f, "seq: {}", input.sequence)?;
        }
